...
import net.lopymine.pc.exception.HttpResponseException;
...
public class MojangAPI {
	...
	public static Response<UUID> getUUID(String nickname, boolean canRetry) {
		boolean debugLogEnabled = PlayerCushionsConfig.getInstance().isDebugLogEnabled();
		int statusCode = -1;
		...
		try {
			...
			if (statusCode == 429) {
				if (debugLogEnabled) {
					PlayerCushionsClient.LOGGER.warn("Received Too Many Requests on {}", nickname);
				}
				return Response.empty(statusCode);
			}
			if (statusCode == 404) {
				if (debugLogEnabled) {
					PlayerCushionsClient.LOGGER.warn("Failed to find player profile with nickname {}", nickname);
				}
				return Response.empty(statusCode);
			}
			...
			PlayerCushionsClient.LOGGER.error("Failed to get UUID: ", e);
			PlayerCushionsClient.LOGGER.error("Response Body: ");
			PlayerCushionsClient.LOGGER.error(responseBody);
		}

		if (statusCode == 403) {
			PlayerCushionsClient.LOGGER.error("Received 403 status code, using fallback API for UUIDs!");
	...
	public static Response<ParsedSkinData> getSkinData(UUID uuid, String nickname) {
		try {
			...
			PlayerCushionsClient.LOGGER.error("Failed to load skin textures for {}: ", nickname, e);
		}
		return null;
	}
